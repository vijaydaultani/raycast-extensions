...
function getCompletedTodayTodos(): Todo[] {
  const jxaScript = `
    ...
    for (const todo of todos) {
      ...
      if (completionDate && completionDate >= today && completionDate < tomorrow) {
...
export default function Command() {
  const { data: todos, isLoading, error } = useCachedPromise(
    () => Promise.resolve(getCompletedTodayTodos()),
    [],
    {
      onError: (error) => {
        showToast({
  ...
  return (
    <List
      ...
      navigationTitle={`Completed Today (${totalCount})`}
    >
      {!todos || todos.length === 0 ? (
        <List.EmptyView
          icon={Icon.CheckCircle}
          title="No tasks completed today"
          description="You haven't completed any tasks in Things today yet"
        />
      ) : (
